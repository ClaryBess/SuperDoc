...
export default {
  ...
  methods: {
    ...
    fetchInfo() {
      var _this = this;
      axios
        .post("/team/getInfo/" + this.$route.params.id)
    ...
    submitFormMember(formName) {
      ...
      this.$msgbox({
        title: "提示",
        message: h("p", null, [h("span", null, "确定修改团队简介吗?")]),
    ...
    fetchLeader() {
      var _this = this;
      axios
        .post("/team/getUser/" + this.$route.params.id)
        .then(function (response) {
    ...
    fetchMember() {
      var _this = this;
      axios
        .post("/team/getMember/" + this.$route.params.id)
        .then(function (response) {
